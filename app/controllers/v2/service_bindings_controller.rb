  ...
  def update
    ...
    begin
      if plan.container_manager.find(instance_guid)
        ...
        cmd = "node /var/vcap/packages/cf-containers-broker/pusher.js -p http://#{host}:#{node_port} -a #{account} -x foo -c '#{JSON.generate(parsedParameters["contract_args"])}' /tmp/simple.bin 2>&1"
